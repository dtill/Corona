...
library(lubridate)
# old link:
# stridat <- fread("https://raw.githubusercontent.com/OxCGRT/covid-policy-tracker/master/data/timeseries/stringencyindex_legacy.csv", na.strings=".")
stridat <- fread("https://raw.githubusercontent.com/OxCGRT/covid-policy-tracker/master/data/timeseries/index_stringency.csv", na.strings=".")
...
# only with more than min_cases confirmed, to allow for senseful fit
countover <- countover[Confirmed>min_cases]
...
```{r TableOne}
kable(head(countover[order(-maxday),c("country","maxday")],50),digits=3)
```
